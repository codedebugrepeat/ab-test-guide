...
function insightFor(liftIndex: number, falseNegativeShare: number): string {
...
export function LiftEffectWidget() {
  ...
  return (
    <div className="flex flex-col items-center gap-6">
      ...
      <BellsThresholdChart pA={pA} pB={pB} n={CH4_N} confidence={CH4_CONFIDENCE} />

      <p className="max-w-[520px] text-center text-sm text-foreground/60">
        {insightFor(liftIndex, readout.falseNegativeShare)}
      </p>
      ...
      <p className="text-[11px] text-foreground/45 tabular-nums">
        ...
        confidence {(CH4_CONFIDENCE * 100).toFixed(0)}%.
      </p>
